import { Result, RunResponse, RunSolution, Submission, SubmitSolution } from "@/types/submission";
import { apiClient } from "../api/api-client";
import { API_CONFIG } from "../api/api-config";

export const SubmissionService = {
  runSolution: async (solution: RunSolution): Promise<RunResponse> => {
    try {
      console.log(`Run solution: ${JSON.stringify(solution)}`);
      const response = await apiClient.post<RunResponse>(`${API_CONFIG.API_END_POINT.SUBMISSION}/run`, solution);
      return response;
    } catch (error) {
      throw error;
    }
  },
  submitSolution: async (solution: SubmitSolution): Promise<Submission> => {
    try {
      console.log(`Submit solution: ${JSON.stringify(solution)}`);
      const response = await apiClient.post<Submission>(`${API_CONFIG.API_END_POINT.SUBMISSION}/submit`, solution);
      return response;
    } catch (error) {
      throw error;
    }
