...
import { ArrowLeft, CheckCircle, Clock, XCircle, ChevronDown, ChevronRight, Trophy, AlertCircle, Loader2, User as UserIcon, Play, Terminal, BookOpenText, BugPlay, Brain } from "lucide-react";
          ...
          <div className="flex items-center space-x-4 text-xs">
            ...
            {submission.time && (
              <div className={`flex items-center space-x-1 ${themeClasses.muted}`}>
                ...
                <span>{submission.time.toFixed(4)} s</span>
              </div>
            )}
            {submission.memory && (
              <div className={`flex items-center space-x-1 ${themeClasses.muted}`}>
                <span>💾</span>
                <span>{submission.memory.toFixed(3)} kb</span>
              </div>
            )}
                    ...
                    <div className={`flex items-center space-x-3 pt-1 ${themeClasses.muted}`}>
                      {result.time != null && (
                        <span>
                          ...
                          {result.time.toFixed(4)} s
                        </span>
                      )}
                      {result.memory != null && (
                        <span>💾 {result.memory.toFixed(3)} kb</span>
                      )}
                      {result.point != null && (
...
export default function ProblemSolvePage() {
  ...
  const [activeTab, setActiveTab] = useState<'descriptions' | 'examples' | 'results' | 'run'>('descriptions')
  ...
  const handleRunCode = async () => {
    if (!user) {
      toast.error("You need to be logged in to run.");
      return;
    } else {
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <Textarea
            ...
            className={`min-h-[120px] font-mono text-sm resize-none ${themeClasses.mutedBg} ${themeClasses.text} ${themeClasses.border}`}
            disabled={isRunning}
          />
        </div>
        ...
        <div className="flex items-center justify-between">
          <Button
            ...
            className={`flex items-center gap-2 text-sm ${themeClasses.muted}`}
            variant="outline"
          >
            {isRunning ? (
              <>
          ...
          <div className="space-y-3">
            ...
            {output.success && output.stdout && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <label className={`text-sm font-medium ${themeClasses.success}`}>
                    Output:
                  </label>
                </div>
                <div className={`border rounded p-3 ${themeClasses.border} ${themeClasses.cardBg} max-h-48 overflow-auto`}>
                  <pre className={`text-sm font-mono whitespace-pre-wrap ${themeClasses.text}`}>
                    {output.stdout}
                  </pre>
                </div>
              </div>
            ...
            {output.stderr && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <label className={`text-sm font-medium ${themeClasses.error}`}>
                    Runtime Error:
                  </label>
                </div>
                <div className={`border border-red-300 dark:border-red-700 rounded p-3 bg-red-50 dark:bg-red-900/20 max-h-48 overflow-auto`}>
                  <pre className={`text-sm font-mono whitespace-pre-wrap ${themeClasses.error}`}>
                    {output.stderr}
                  </pre>
                </div>
              </div>
            ...
            {output.compile_output && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <label className={`text-sm font-medium ${themeClasses.warning}`}>
                    Compilation Error:
                  </label>
                </div>
                <div className={`border border-yellow-300 dark:border-yellow-700 rounded p-3 bg-yellow-50 dark:bg-yellow-900/20 max-h-48 overflow-auto`}>
                  <pre className={`text-sm font-mono whitespace-pre-wrap ${themeClasses.warning}`}>
                    {output.compile_output}
                  </pre>
                </div>
              </div>
            ...
            {output.message && !output.success && (
              <div>
                ...
                <div className={`border border-orange-300 dark:border-orange-700 rounded p-3 bg-orange-50 dark:bg-orange-900/20`}>
            ...
            {/* Empty output case */}
            {!output.stdout && !output.stderr && !output.compile_output && output.success && (
              <div className={`text-center py-4 ${themeClasses.muted}`}>
        ...
        {!output && !isRunning && (
          <div className={`text-center py-8 border-2 border-dashed rounded-lg ${themeClasses.border} ${themeClasses.muted}`}>
            <Terminal className="h-8 w-8 mx-auto mb-2 opacity-50" />
            <p className="text-sm mb-1">Ready to test your code</p>
            <p className="text-sm opacity-75">Enter input and click Run</p>
          </div>
        )}
      </div>
    </TabsContent>
  ...
  const renderLeftPanel = () => (
    <ResizablePanel defaultSize={40} minSize={30}>
      <div className={`h-full ${themeClasses.background} overflow-auto p-6`}>
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as ('descriptions' | 'examples' | 'results' | 'run'))} className={isDark ? 'dark' : ''}>
          <TabsList className={`grid w-full grid-cols-4 ${themeClasses.mutedBg} ${themeClasses.border}`}>
            <TabsTrigger className={`data-[state=active]:${themeClasses.cardBg} data-[state=active]:${themeClasses.text}`} value="descriptions">
              <Brain className="h-3 w-3 mr-1" />
              Description
            </TabsTrigger>
            <TabsTrigger className={`data-[state=active]:${themeClasses.cardBg} data-[state=active]:${themeClasses.text}`} value="examples">
              <BookOpenText className="h-3 w-3 mr-1" />
              Examples
            </TabsTrigger>
            <TabsTrigger className={`data-[state=active]:${themeClasses.cardBg} data-[state=active]:${themeClasses.text}`} value="run">
              <Terminal className="h-3 w-3 mr-1" />
              Run
            </TabsTrigger>
            <TabsTrigger className={`data-[state=active]:${themeClasses.cardBg} data-[state=active]:${themeClasses.text}`} value="results">
